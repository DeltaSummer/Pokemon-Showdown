...
let customColors = FS('server/impulse/database/customcolors.json').readIfExistsSync();
if (customColors !== '') {
...
function updateColor() {
	FS('server/impulse/database/customcolors.json').writeUpdate(() => (
...
exports.commands = {
	...
	customcolor: {
		set: function (target, room, user) {
			...
			if (target[0].length > 19) return this.errorReply("Usernames are not this long...");
